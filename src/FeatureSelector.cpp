...
void FeatureSelector::learnEffectiveProperties() {
    ...
    for (int k=0; k<=1; k++) {
        ...
        for ( ; it != end_it; it++) {
            ...
            // this action-attribute pairing algorithm is interesting
            // part 1
            if (k == 0) {
                if (quality_of_property.find(property_name) == quality_of_property.end()) {
                    quality_of_property[property_name] = quality; 
                    action_effective_[property_name] = action_name; 
                } 
                else if (quality < quality_of_property[property_name]) {
                    quality_of_property[property_name] = quality; 
                    action_effective_[property_name] = action_name; 
                }
            }
    ...
    // part 2
    for (std::map<std::string, std::string>::iterator it = action_effective_.begin(); 
            it != action_effective_.end(); it++) {
        property_effective_[it->second] = it->first; 
    }
